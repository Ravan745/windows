...
class MusicBot extends Client {
  constructor() {
    ...
    this.ankushcolor = this.config.ankushcolor;
  ...
  _loadClientEvents() {
    readdirSync("./src/events/Client").forEach(file => {
      ...
      if (event.name === "rateLimit" || event.name === "rateLimited") {
        this.rest.on("rateLimited", (...args) => event.run(this, ...args));
      } else {
  ...
  _loadSlashCommands() {
    ...
    this.on("ready", async () => {
      if (process.argv.includes('--deploy')) {
        this.logger.log("Registering slash commands to Discord...", "cmd");
        ...
        this.logger.log("Skipped automatic slash command registration (use --deploy flag or !deploy command to register)", "cmd");
      }
    });
  }
  connect() {
