...
const getAccessToken = async (client) => {
  ...
  try {
    const clientId = client?.config?.SpotifyID || process.env.SPOTIFY_ID || 'e6f84fbec2b44a77bf35a20c5ffa54b8';
    const clientSecret = client?.config?.SpotifySecret || process.env.SPOTIFY_SECRET || '498f461b962443cfaf9539c610e2ea81';
...
const fetchSpotifyProfile = async (client, spotifyUrl) => {
...
const fetchPlaylistDetails = async (client, playlistId) => {
    const token = await getAccessToken(client);
    if (!token) return null;
...
const fetchUserPlaylists = async (client, spotifyUrl) => {
  const userId = extractUserId(spotifyUrl);
  if (!userId) {
    console.error("Invalid Spotify URL");
    return null;
...
const searchSpotifyPlaylists = async (client, query) => {
  const token = await getAccessToken(client);
  if (!token) return null;
...
module.exports = {
    ...
    if (args[0].toLowerCase() === "login") {
      ...
      const profileData = await fetchSpotifyProfile(client, url);
      if (!profileData) {
    ...
    if (args[0].toLowerCase() === "profile") {
      ...
      const profileData = await fetchSpotifyProfile(client, userProfile.profileUrl);
      if (!profileData) {
    ...
    if (args[0].toLowerCase() === "playlist") {
      ...
      const playlists = await fetchUserPlaylists(client, userProfile.profileUrl);
      ...
      collector.on("collect", async (interaction) => {
        if (interaction.isStringSelectMenu() && interaction.customId === "playlist_select") {
          ...
          const playlistDetails = await fetchPlaylistDetails(client, selectedPlaylistId);
          ...
          actionCollector.on("collect", async (actionInteraction) => {
            if (actionInteraction.isStringSelectMenu() && actionInteraction.customId.startsWith("action_")) {
              ...
              if (action === "play") {
                ...
                const playlistDetails = await fetchPlaylistDetails(client, playlistId);
                if (!playlistDetails) {
    ...
    if (args[0].toLowerCase() === "searchplaylist") {
      ...
      const searchResults = await searchSpotifyPlaylists(client, searchQuery);
